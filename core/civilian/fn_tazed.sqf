...
//CONTROLLO BERSAGLIO A TERRA
if(_unit getVariable "FAR_isUnconscious" == 1) exitWith {life_istazed = false;};
if(animationState _unit == "AinjPpneMstpSnonWrflDnon") exitWith {life_istazed = false;};
...
if(_shooter isKindOf "Man" && alive player) then
{
	...
	{
		life_istazed = true;
		//player setVariable ["IStazed",1,TRUE];
		...
		[[0,format["%1 è stato tazerato da %2", _unit getVariable["realname",name _unit], _shooter getVariable["realname",name _shooter]]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		...
		life_istazed = false;
		//player setVariable ["IStazed",0,TRUE];
