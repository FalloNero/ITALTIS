...
if((_house isKindOf "House_F")OR(_house isKindOf "House_EP1")) then
{
	_owner = _house getVariable["life_homeOwnerName", []];
	[[0,format["%1, e' in corso una perquisizione nella tua abitazione!", (_owner select 0)]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	_displayName = getText(configFile >> "CfgVehicles" >> (typeOf _house) >> "displayName");
	_upp = format["Sfondando %1",_displayName];
	...
	titleText["Hai aperto la casa.","PLAIN"];
};