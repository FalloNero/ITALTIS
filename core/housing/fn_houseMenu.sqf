...
if(((_curTarget isKindOf "House_F")||(_curTarget isKindOf "House_EP1")) && (playerSide == west or playerSide == east)) exitWith {
...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget getVariable "house_owner"}) then {
	...
	_Btn2 ctrlSetText localize "STR_pInAct_BreakDown";
	_Btn2 buttonSetAction "[life_pInact_curTarget] spawn life_fnc_civBreakDoor; closeDialog 0;";
	_Btn2 ctrlShow true;
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F","Land_e76_us_garage01"]) then {
		...
		if(life_pInact_curTarget in life_vehicles OR {!(life_pInact_curTarget getVariable ["locked",true])}) then
		{
			_Btn4 ctrlSetText "INVENTARIO CASA";
			_Btn4 buttonSetAction "[life_pInact_curTarget] call life_fnc_openInventory;";
			_Btn4 ctrlShow true;			
		};
	};
};