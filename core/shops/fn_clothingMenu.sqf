...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart","merc","donator"] && playerSide != civilian) exitWith {hint "Devi essere un civile per utilizzare questo shop!"; closeDialog 0;};
//if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Non sei abilitato a questo shop!"; closeDialog 0;};
	
	
if((_this select 3) in ["cop"])then {
	
	if(playerSide != west) then {
		hint "Non sei un poliziotto!"; closeDialog 0;	
	};
		
};

if((_this select 3) in ["opfor"])then {
...
//if((_this select 3) == "opfor" && !license_cop_copmessico) exitWith {hint "Non sei pulotto idiota..."; closeDialog 0;};	
//if((_this select 3) == "cop" && !license_cop_copamericano) exitWith {hint "Non sei pulotto idiota..."; closeDialog 0;};		
//if((_this select 3) == "merc" && !license_civ_merc) exitWith {hint "You don't have merc training yet!"; closeDialog 0;};
//if((_this select 3) == "mafia" && !license_civ_mafia) exitWith {hint "Non sei un blackwater!"; closeDialog 0;};
//if((_this select 3) == "soa" && !license_civ_soa) exitWith {hint "Non sei un SOA"; closeDialog 0;};
//if((_this select 3) == "sha" && !life_shadow) exitWith {hint "Fottiti"; closeDialog 0;};
//if((_this select 3) == "aod" && !license_civ_aod) exitWith {hint "Non sei un Aod"; closeDialog 0;};
//if((_this select 3) == "narcos" && !license_civ_narcos) exitWith {hint "Non sei un Narcos"; closeDialog 0;};
if((_this select 3) == "mafia" && !license_civ_mafia) exitWith {hint "Non sei un mafioso"; closeDialog 0;};
if((_this select 3) == "spartan" && !license_civ_spartan) exitWith {hint "Non sei uno spartan"; closeDialog 0;};
