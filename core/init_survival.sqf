...
[] spawn
{
    ...
    while{true} do
    {
        ...
        if((playerSide == west) or (PlayerSide==civilian) or (PlayerSide==independent)) then {(unitBackpack player) setObjectTextureGlobal [0,""];};
	...
	while{true} do 
	{
		if (alive player) then
		{
			if (player getVariable ["FAR_isUnconscious",0] == 0)then
			{
				...
				{
					_text = format["%1 è un figlio di puttana.ID del figlio di puttana = %2 (FUORI MAPPA).COORDINATE : %3",profileName,getPlayerUID player,_pos];
					[[_text],"life_fnc_logging",false,false] spawn life_fnc_MP;
					vehicle player setDamage 1;
				}else{
					if (((_pos select 0) > 10239.9)||((_pos select 1) > 10239.3)) then
					{
						_text = format["%1 è un figlio di puttana.ID del figlio di puttana = %2 (FUORI MAPPA).COORDINATE : %3",profileName,getPlayerUID player,_pos];
						[[_text],"life_fnc_logging",false,false] spawn life_fnc_MP;
						vehicle player setDamage 1;
