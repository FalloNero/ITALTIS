...
_baseprice = 5000;  //Price for repaint
_vehicleData = _veh getVariable["vehicle_info_owners",[]];
_vehOwner = (_vehicleData select 0) select 0;
if(life_cash < _basePrice) exitWith {hint "Non hai denaro sufficiente"};

if(isNil {_vehicleData}) exitWith {hint "Veicolo non riconosciuto."};
if ((getPlayerUID player) != _vehOwner) exitWith {hint "Non sei il proprietario"};

		life_action_inUse = true;
		_displayName = getText(configFile >> "CfgVehicles" >> (typeOf _veh) >> "displayName");
		_upp = format["Colorazione veicolo %1",_displayName];
		...
		if(player != vehicle player) exitWith {titleText["Devi uscire dal veicolo per dipingerlo","PLAIN"];};
