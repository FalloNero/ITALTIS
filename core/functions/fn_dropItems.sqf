	...
	switch(_item) do
	{
		...
		case "life_cash":
		{
			if(life_cash > 0) then
			{
				_pos = _unit modelToWorld[0,3,0];
				//_pos = [_pos select 0, _pos select 1, 0];
				_value = life_cash - life_bank_cash ;
				if (_value != 0) then {
				...
				missionNamespace setVariable[_x,0];
				if (life_bank_cash > 0)then{
					_pos = _unit modelToWorld[0,3,0];
					//_pos = [_pos select 0, _pos select 1, 0];
					_obj = "il_money_stack_20" createVehicle _pos;
					_obj setPos _pos;
					_value = life_bank_cash;
					_obj setVariable["item",["bank_money",_value],true];
				};
			};
		};
