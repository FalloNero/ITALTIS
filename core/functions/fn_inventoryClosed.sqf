...
if((typeOf _container) in ["Box_IND_Grenades_F","B_supplyCrate_F"]) exitWith {
	...
	switch(true) do {
		...
		default {
			{if((_x isKindOf "House_F")OR(_x isKindOf "House_EP1")) exitWith {_house = _x;};} foreach _house;
		};
	};
	if(!isNil "_exit" OR !((_house isKindOf "House_F")OR(_house isKindOf "House_EP1"))) exitWith {systemChat localize "STR_House_ContainerError"};
