...
while{true} do 
{
	...
	if(isEngineOn _vehicleToFuel && ((driver _vehicleToFuel) == player) && (_vehicleToFuel != player) ) then
	{
		...
		_fuelConsumption = _velocityOfVehicle/30000 + 0.0001;
		...
		{
			hint "Sei in riserva! corri a rifornirti";
		}
		else
		{
			if(fuel _vehicleToFuel < 0.03) then
			{
				hint "Spero che tu sappia camminare, perchè hai fitino la benzina";
			};
		};
	};
	sleep 2;
